...
class UI:
    ...
    def left_clicked(self):
        self.board.falling_piece.shift(-1)
    ...
    def update_board(self):
        ...
        if not self.board.update_falling():
            ...
            p.shift(random.randint(0, self.board.max_col - 4))  # -4 to keep the whole piece within the board area
            # make it fall
            self.board.set_falling_piece(p)

        self.refresh_canvas()  # TODO: fix bug printing piece after the game over
        self.window.after(500, self.update_board)


if __name__ == '__main__':
    game = UI(TetrisBoard(10, 10))
